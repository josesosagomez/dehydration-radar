...
- `summarize_exp_c(results, config)` — per arm: pooled class-unit MAE, adjacent accuracy, QWK
  ...
  single-valued, i.e. the folds option 8b would have skipped) and `n_qwk_nan` (**distinct**
  validation folds where at least one evaluated QWK was genuinely undefined) at both CV levels.
  Because undefinedness depends on predictions and several predictions are evaluated per fold,
  each level also reports `n_qwk_nan_evaluation_cells` and `n_qwk_evaluation_cells`, with the cell
  unit stated explicitly: stage × candidate × inner fold at the inner level, and arm × realized
  seed × outer fold at the outer level. These accompany the bootstrap's existing skip-and-count
  for the QWK CI** — so §8 can state how much the (8a) choice actually changed rather than
  asserting it was negligible;
...
| T-M9-expc-report | test_exp_c.py, test_run_ordinal.py | Arm (a)/(b) both summarized; pooled seed-collapse used for all three CIs; confusion = per-seed counts averaged; per-subject distribution present; `conditional_exploratory` true; no baseline-comparison field; **the O-M9-8 fold counters (`n_single_class_truth_val_folds`, `n_qwk_nan`) and candidate/arm-seed evaluation-cell numerators and denominators present at both CV levels, with a fixture proving two undefined candidate cells on one validation fold report `n_qwk_nan = 1`, not 2**; smoke surfaces no performance token; RNG offsets pairwise distinct incl. Exp B's and D's |
